...
def main():
    ...
    for epoch in range(5):
        for x, y in train_loader:
            ...
            # >>> changed: randomly select 1/4 samples
            batch_size = x.size(0)
            k = batch_size // 4
            ...
            # >>> changed: PGD only on selected samples
            x_adv = pgd_attack(model, x_sel, y_sel)

            # >>> changed: use only adversarial subset for loss
            optimizer.zero_grad()
            logits = model(x_adv)
            loss = nn.CrossEntropyLoss()(logits, y_sel)
            loss.backward()
            optimizer.step()
